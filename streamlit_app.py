...
# Churn shares for reminder scenarios\churn_day4 = pd.DataFrame({
...
# حالت Custom Mix
    ...
    # NLI custom
    rem_nli_c = st.sidebar.radio("Reminder Day for NLI (Custom)", ("Day 4", "Day 5"), key="rem_nli_c")
    shares_nli_c = nli_day4 if rem_nli_c == "Day 4" else nli_day5
    sel_nli = st.sidebar.multiselect("انتخاب Segments NLI", options=list(nli_map.keys()), key="sel_nli_c")

    # Churn custom
    rem_ch_c = st.sidebar.radio("Reminder Day for Churn (Custom)", ("Day 4", "Day 5"), key="rem_ch_c")
    shares_ch_c = churn_day4 if rem_ch_c == "Day 4" else churn_day5
    sel_ch = st.sidebar.multiselect("انتخاب Churn Groups", options=list(churn_map.keys()), key="sel_ch_c")

    total_df = pd.Series(0.0, index=range(1,8), name="Total Orders")

    # محاسبه برای NLI custom
    for lbl in sel_nli:
        idx = nli_map[lbl]
        sz = st.sidebar.number_input(f"Size {lbl}", min_value=1, value=1000, step=100, key=f"sz_{lbl}")
        crp = st.sidebar.slider(f"CR {lbl}", min_value=0.0, max_value=100.0, value=5.0, step=0.5, format="%.1f%%", key=f"cr_{lbl}")
        total_df += shares_nli_c[idx] / 100 * (sz * crp / 100)

    # محاسبه برای Churn custom
    for lbl in sel_ch:
        idx = churn_map[lbl]
        sz = st.sidebar.number_input(f"Size {lbl}", min_value=1, value=500, step=50, key=f"sz_{lbl}_c")
        crp = st.sidebar.slider(f"CR {lbl}", min_value=0.0, max_value=100.0, value=2.0, step=0.5, format="%.1f%%", key=f"cr_{lbl}_c")
        total_df += shares_ch_c[idx] / 100 * (sz * crp / 100)
